...
import {blogCreateModel, blogCreateModelWrongName} from "../utils/blogs-options";
...
describe('BLOGS (e2e)', () => {
    ...
    it('/blogs (POST), should returned status 201 and correct blog model', async () => {
        const responseCreateBlog = await blogsTestManager.createBlog(blogCreateModel, authBasic)
    ...
    it('/blogs (POST), should returned status 401', async () => {
        const responseCreateBlog = await blogsTestManager.createBlog(blogCreateModel, authBasicWrong)
